...
class ModelInteraction(nn.Module):
    def __init__(self, embedding, contact, use_cuda, do_pool=True, pool_size=9, theta_init=1, lambda_init = 0, gamma_init = 0, use_W=True, do_sigmoid = False, use_glider=False, glider_map={}, glider_mat=None, glider_param=0.5):
    ...
    def set_glider(self, glider_mat, glider_param, glider_map):
        self.glider_map = glider_map
        self.glider_mat = glider_mat
        self.glider_param = glider_param
        return

    def glider_score(self, prot1, prot2):
        for m in [prot1, prot2]:
            if m not in self.glider_map:
                return 0
        prot_1_map = self.glider_map[prot1]
        prot_2_map = self.glider_map[prot2]
        return self.glider_mat[prot_1_map, prot_2_map]
